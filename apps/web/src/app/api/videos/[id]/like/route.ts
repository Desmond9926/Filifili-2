...
export const POST = withErrorHandling(async (req: NextRequest, ctx?: unknown) => {
  ...
  if (!existing) {
    await prisma.$transaction([
      prisma.like.create({ data: { userId: auth.userId, videoId } }),
      prisma.video.update({ where: { id: videoId }, data: { likeCount: { increment: 1 } } })
    ]);
  }
...
export const DELETE = withErrorHandling(async (req: NextRequest, ctx?: unknown) => {
  ...
  if (existing) {
    await prisma.$transaction([
      prisma.like.delete({ where: { userId_videoId: { userId: auth.userId, videoId } } }),
      prisma.video.update({ where: { id: videoId }, data: { likeCount: { decrement: 1 } } })
    ]);
  }
  return NextResponse.json(ok({ success: true }));
});
